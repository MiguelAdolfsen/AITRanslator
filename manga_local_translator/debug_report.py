...
def build_page_debug_summary(
    ...
    return {
        ...
        "cat_retry_attempted": sum(1 for context in contexts if context.get("cat_retry_attempted") is True),
        "cat_retry_accepted": sum(1 for context in contexts if context.get("cat_retry_accepted") is True),
        "cat_retry_rejected": sum(1 for context in contexts if context.get("cat_retry_attempted") is True and context.get("cat_retry_accepted") is not True),
        "cat_retry_reject_reasons": dict(count_values(context.get("cat_retry_reject_reason") for context in contexts if context.get("cat_retry_attempted") is True and context.get("cat_retry_accepted") is not True)),
        "cat_suspect_second_pass_attempted": sum(1 for context in contexts if context.get("cat_suspect_second_pass_attempted") is True),
        "cat_suspect_second_pass_accepted": sum(1 for context in contexts if context.get("cat_suspect_second_pass_accepted") is True),
        "cat_suspect_second_pass_rejected": sum(1 for context in contexts if context.get("cat_suspect_second_pass_attempted") is True and context.get("cat_suspect_second_pass_accepted") is not True),
        "cat_suspect_second_pass_reject_reasons": dict(count_values(context.get("cat_suspect_second_pass_reject_reason") for context in contexts if context.get("cat_suspect_second_pass_attempted") is True and context.get("cat_suspect_second_pass_accepted") is not True)),
